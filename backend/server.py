...
db_name = os.environ.get('DB_NAME', 'resumee')
db = client[db_name]
...
def _load_semantic_model():
    ...
    try:
        ...
        _semantic_available = False
        logger.warning("⚠️  [Semantic] sentence-transformers not installed — using token-overlap fallback. Install with: pip install sentence-transformers scikit-learn")
    return _semantic_available
...
async def analyze_resume(request: AnalyzeRequest):
    ...
    try:
        ...
        logger.info(f"[Analysis] manual analyze_resume completed in {processing_time_seconds}s")
        return {
...
async def analyze_uploaded_resume(
    ...
    try:
        ...
        logger.info(f"[Analysis] analyze_uploaded_resume completed in {processing_time_seconds}s for resume_id={resume_id}")
        return {
...
async def advanced_single_scan(
    ...
    try:
        ...
        processing_time_seconds = round(time.perf_counter() - start_time, 4)
        await db.resumes.insert_one({**resume_data, "processing_time_seconds": processing_time_seconds})
