...
const SingleUploadPage = () => {
  ...
  const handleAnalyze = async () => {
    ...
    setIsAnalyzing(true); setResults(null); setProgress(10); setElapsedSeconds(0);
    const startTimer = setInterval(() => setElapsedSeconds((prev) => prev + 1), 1000);
    try {
            ...
            {isAnalyzing && (
              <Card className="border-0 shadow-sm h-full flex items-center justify-center min-h-[500px]">
                <CardContent className="text-center py-16 space-y-4">
                  ...
                  <p className="text-sm text-gray-700">Elapsed: <span className="font-semibold">{elapsedSeconds}s</span></p>
                ...
                <Card className="border-0 shadow-sm overflow-hidden">
                  <div className={`p-8 ${scoreColor(results.ats_score)}`}>
                    <div className="flex items-center justify-between">
                      ...
                      {results.processing_time_seconds != null && (
                        <p className="text-xs text-gray-700 mt-2">Processed in <span className="font-semibold">{results.processing_time_seconds}s</span></p>
                      )}
                      </div>
